...
if ($STEP == 1)
{
	...
	<tr>
		<td valign="top" width="40%">Файл импорта - </td>
...
?>
	<tr>
		<td valign="top" width="40%">Торговый каталог</td>
		<td valign="top" width="60%">
		<?
			if (!isset($IBLOCK_ID))
				$IBLOCK_ID = 0;
	...
	<tr>
		<td valign="top" width="40%">С сайта:</td>
		<td valign="top" width="60%">
		<?foreach ($arSitesImport as $strAbbr => $strName):?>
			<input
				type="radio"
				name="SITE_NAME_ABBR"
				id="LABEL_<?=$strAbbr?>"
				value="<?=$strAbbr?>"
				<?if ($SITE_NAME_ABBR == $strAbbr) echo "checked";?>> 
			<label for="LABEL_<?=$strAbbr?>"><?=$strName?></label><br> 
		<?endforeach;?>
		</td>
	</tr>
		<tr>
		<td valign="top" width="40%">Наценка:</td>
		<td valign="top" width="60%">
			<input type="text" name="MARKUP" size="40" value="<? echo htmlspecialcharsbx($MARKUP);?>">
		</td>
	</tr>
	<tr>
		<td valign="top" width="40%">Конвертировать в UTF-8:</td>
		<td valign="top" width="60%"><?
	...
	<?if ($ACTION=="IMPORT_SETUP" || $ACTION == 'IMPORT_EDIT' || $ACTION == 'IMPORT_COPY')
	{
		...
		<tr>
			<td valign="top" width="40%">Наименование:</td>
			<td valign="top" width="60%">
